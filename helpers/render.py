...
# Subtitle presets. EDL field `subtitle_style` selects one of these.
# `fontsdir` in the EDL (or edit/fonts/) supplies custom faces like Manrope.
SUBTITLE_PRESETS: dict[str, dict[str, str | int]] = {
    ...
    "manrope-speech": {
        "force_style": (
            "FontName=Manrope,FontSize=12,Bold=1,"
            "PrimaryColour=&H00FFFFFF,OutlineColour=&H00000000,BackColour=&H00000000,"
    ...
    "manrope-speech-dark": {
        "force_style": (
            "FontName=Manrope,FontSize=22,Bold=1,"
            "PrimaryColour=&H00000000,OutlineColour=&H00000000,BackColour=&H80000000,"
...
def resolve_subtitle_config(edl: dict, edit_dir: Path) -> tuple[str, dict]:
    """Return (force_style, chunking config) from EDL subtitle fields."""
    style_name = edl.get("subtitle_style") or "bold-overlay"
    preset = SUBTITLE_PRESETS.get(style_name, SUBTITLE_PRESETS["bold-overlay"]).copy()
    if edl.get("subtitle_force_style"):
        preset["force_style"] = edl["subtitle_force_style"]
    if edl.get("subtitle_chunk_words"):
...
def resolve_fontsdir(edl: dict, edit_dir: Path) -> Path | None:
    """Return fonts directory for libass, if Manrope (or other custom face) is needed."""
    if edl.get("fontsdir"):
        p = Path(edl["fontsdir"])
        return p if p.is_absolute() else (edit_dir / p).resolve()
    default = edit_dir / "fonts"
    return default if default.is_dir() and any(default.iterdir()) else None
...
# zscale not available in this build; use colorspace filter for BT.2020 → BT.709
TONEMAP_CHAIN = "colorspace=iall=bt2020:all=bt709:format=yuv420p"
...
def extract_all_segments(
    ...
    scale_override = edl.get("scale") or None
    clips_dir = edit_dir / (
    ...
    for i, r in enumerate(ranges):
        ...
        extract_segment(src_path, start, duration, seg_filter, out_path, preview=preview, draft=draft, scale_override=scale_override)
...
def build_master_srt(
    ...
    *,
    ...
    for r in edl["ranges"]:
        ...
        # Group into chunks; break early on punctuation
...
def build_final_composite(
    ...
    *,
    ...
    if has_subs:
        ...
        fonts_opt = ""
        if fontsdir and fontsdir.is_dir():
            fonts_abs = str(fontsdir.resolve()).replace(":", r"\:").replace("'", r"\'")
            fonts_opt = f":fontsdir='{fonts_abs}'"
        filter_parts.append(
            f"{current}subtitles='{subs_abs}'{fonts_opt}:force_style='{force_style}'[outv]"
        )
        out_label = "[outv]"
    else:
...
def main() -> None:
    ...
    force_style, sub_cfg = resolve_subtitle_config(edl, edit_dir)
    fontsdir = resolve_fontsdir(edl, edit_dir)
    ...
    if args.no_loudnorm:
        build_final_composite(
            ...
            force_style=force_style, fontsdir=fontsdir,
        )
    else:
